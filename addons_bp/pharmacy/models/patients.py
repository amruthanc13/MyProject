...
class PharmacyPatientInherit(models.Model):
    ...
    copayment = fields.Float(string='Co-payment', default=0,
                             help="Indicates the percentage of amount "
                             "paid by the customer")

    @api.constrains('copayment')
    def check_copayment(self):
        if not (0.0 <= self.copayment <= 100.0):
            raise ValidationError(
                _("Co-payment must be between 0 and 100"))

    def print_report(self):
        return self.env.ref(
            'pharmacy.report_patient_clinical_history').report_action(self)
